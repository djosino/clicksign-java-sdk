...
public final class BulkRequirement {
    ...
    public static final class Operations {
        ...
        public Operations addProvideEvidence(String signerId, String documentId, String auth) {
            validateIds(signerId, documentId);
            if (auth == null || auth.isBlank()) throw new IllegalArgumentException("auth is required");
        ...
        public Operations addRubricate(String signerId, String documentId,
            ...
            validateIds(signerId, documentId);
            if (pages == null && rubricField == null) throw new IllegalArgumentException("pages or rubricField is required");
            ...
            if (pages       != null) attrs.put("pages",        pages);
            if (rubricField != null) attrs.put("rubric_field", rubricField);
            if (kind        != null) attrs.put("kind",         kind);
        ...
        List<Map<String, Object>> entries() { return atomicOps.entries(); }

        String toJson() { return atomicOps.toJson(); }

        private static void validateIds(String signerId, String documentId) {
            if (signerId == null || signerId.isBlank())   throw new IllegalArgumentException("signerId is required");
    ...
    public static final class Response {
        ...
        public List<OperationResult> results(){ return results; }

        public boolean isSuccess() {
            for (OperationResult r : results) if (!r.isSuccess()) return false;
            return true;
        }

        public List<Requirement> requirements() {
            List<Requirement> list = new ArrayList<>();
            for (OperationResult r : results) if (r.requirement() != null) list.add(r.requirement());
            return Collections.unmodifiableList(list);
        }

        public List<OperationResult> failures() {
            List<OperationResult> list = new ArrayList<>();
            for (OperationResult r : results) if (!r.isSuccess()) list.add(r);
            return Collections.unmodifiableList(list);
        }
    }
    ...
    public static final class Service {
        ...
        public Service(BulkOperationsClient bulkClient) { this.bulkClient = bulkClient; }
                ...
                if (errorsObj instanceof List) {
                    ...
                    for (Object e : (List<?>) errorsObj) {
                        if (e instanceof Map) {
                            ...
                            if (detail == null) detail = ((Map<String, Object>) e).get("title");
                            if (detail != null) {
